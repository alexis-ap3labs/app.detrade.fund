...
export const GET: RequestHandler = async ({ url, params }) => {
  try {
    ...
    // Trouver les deux points les plus proches de 30 jours
    ...
    // Calculer l'interpolation linéaire pour obtenir la valeur à exactement 30 jours
    const targetTimestamp = sortedEvents[0].blockTimestamp - (30 * 24 * 60 * 60);
    ...
    // Calculer la durée exacte de 30 jours en secondes
    ...
    return json({
      ...
      calculationDetails: {
        ...
        formula: {
          ...
          aprFormula: `(${(periodReturn * 100).toFixed(6)}% / ${durationInYears} years) * 100 = ${calculatedApr}% (extrapolé sur 1 an, basé sur exactement 30 jours)`
        }
      }
    });
  } catch (error) {
